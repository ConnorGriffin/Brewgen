...
const getters = {
  ...
  currentStyleSensoryEdit: state => sensoryName => {
    return state.currentStyleSensoryEdit.find(
      sensoryData => sensoryData.name === sensoryName
    );
  },
  ...
  lastSensoryData: state => sensoryName => {
    return state.lastSensoryData.find(
      sensoryData => sensoryData.name === sensoryName
    );
  },
  ...
  fermentableCategoryUsageModified: state => {
    ...
    if (matchCategory) {
      if (
        matchCategory.min_percent !==
          state.fermentableCategoryUsageEdit.minUsage ||
        matchCategory.max_percent !==
          state.fermentableCategoryUsageEdit.maxUsage ||
        matchCategory.unique_fermentable_count !==
          state.fermentableCategoryUsageEdit.uniqueFermentableCount
      ) {
        return true;
      } else {
        return false;
...
const actions = {
  async fetchFermentableCategories({ commit }) {
    return axios
      .get(
        "http://dev.home.connorcg.com:5000/api/v1/style-data/grains/categories"
      )
      .then(response => {
  ...
  async fetchAllFermentables({ commit }) {
    return axios
      .get("http://dev.home.connorcg.com:5000/api/v1/grains")
      .then(response => {
  ...
  async fetchSensoryData({ commit }) {
    ...
    return axios
      .post(
        "http://dev.home.connorcg.com:5000/api/v1/grains/sensory-profiles",
        {
          fermentable_list: fermentable_list,
          category_model: state.fermentableCategories,
          sensory_model: sensoryModel,
          max_unique_fermentables: Number(
            state.equipmentProfile.maxUniqueFermentables
          )
        }
      )
      .then(response => {
  ...
  async fetchSensoryDataEdit({ commit }, name) {
    ...
    let sensoryModel = state.currentStyleSensory
      .filter(sensoryData => {
        return (
          sensoryData.configured !== undefined && sensoryData.name !== name
        );
      })
      .map(sensoryData => {
        return {
  ...
  async fetchRecipeData({ commit }, { colorOnly }) {
    ...
    if (colorOnly === true) {
      ...
      var uri =
        "http://dev.home.connorcg.com:5000/api/v1/grains/recipes?coloronly=true" +
        params;
      ...
      var uri = "http://dev.home.connorcg.com:5000/api/v1/grains/recipes";
  ...
  async fetchStyles({ commit }) {
    ...
    return axios
      .get("http://dev.home.connorcg.com:5000/api/v1/styles")
      .then(response => {
  ...
  async setDataFromStyle({ commit }, styleSlug) {
    commit("resetData");
    return axios
      .get("http://dev.home.connorcg.com:5000/api/v1/styles/" + styleSlug)
      .then(response => {
        ...
        commit(
          "setMaxUniqueFermentables",
          response.data.unique_fermentable_count
        );
  ...
  async fetchFermentableModelValidity({ commit }) {
    return axios
      .post(
        "http://dev.home.connorcg.com:5000/api/v1/helpers/grain-model-valid",
        {
...
const mutations = {
  ...
  setCurrentStyleSensory: (state, sensoryData) => {
    ...
    state.currentStyleSensory = sensoryData
      .filter(sensoryValue => {
        return (
          sensoryValue.max > 0.25 && sensoryValue.max - sensoryValue.min > 0.25
        );
      })
      .map(sensoryValue => {
        let sensoryReturn = {
        ...
        if (
          tagValues.includes("mentioned in style") ||
          tagValues.includes("wide range")
        ) {
  ...
  setPossibleSensory: (state, sensoryData) => {
    ...
    sensoryData.forEach(sensoryValue => {
      ...
      let matchedSensory = state.currentStyleSensory.find(
        csSensory => csSensory.name === sensoryValue.name
      );
  ...
  setPossibleSensoryEdit: (state, sensoryData) => {
    ...
    state.currentStyleSensoryEdit = JSON.parse(
      JSON.stringify(state.currentStyleSensory)
    );
    sensoryData.forEach(sensoryValue => {
      // Find the matching sensory object in the currentStyleSensoryEdit data so we can update it
      let matchedSensory = state.currentStyleSensoryEdit.find(
        csSensory => csSensory.name === sensoryValue.name
      );
      if (matchedSensory !== undefined) {
        Object.assign(matchedSensory, {
  ...
  removeVisibleSensoryDescriptor: (state, name) => {
    if (state.visibleSensoryDescriptors.includes(name)) {
      state.visibleSensoryDescriptors.splice(
        state.visibleSensoryDescriptors.indexOf(name),
        1
      );
    }
  },
  ...
  setFermentableEnabled: (state, { slug, enabled }) => {
    var matchFermentable = state.allFermentables.find(
      fermentable => fermentable.slug === slug
    );
  ...
  removeSensoryConstraint(state, name) {
    let sensoryObj = state.currentStyleSensory.find(
  ...
  setLastSensoryData(state) {
    state.lastSensoryData = JSON.parse(
      JSON.stringify(state.currentStyleSensory)
    );
  },
